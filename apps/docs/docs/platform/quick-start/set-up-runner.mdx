...
Here is an example of how to run the `refreshPluginCache` action using API.

```
curl '<PUBLIC_RUNNER_URL>/v1/plugins/connery-io/connery-runner-administration@main/actions/refreshPluginCache/run' \
-H 'Content-Type: application/json' \
-H 'x-api-key: <CONNERY_RUNNER_API_KEY>' \
-d '{}' | jq .
```
