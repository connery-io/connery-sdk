...
- [What is a runner?](/docs/platform/introduction/core-concepts#runner)
...
Use the link below to create a new GitHub Codespace from the [connery-io/connery-platform](https://github.com/connery-io/connery-platform) repository.
...
[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://github.com/codespaces/new/connery-io/connery-platform?quickstart=1)
...
After the codespace is created, you will see that the `.env` file in the repository's root folder is already created with a unique `CONNERY_RUNNER_API_KEY`.
...
Currently, the runner is running in your codespace and accessible only for you.
...
Here is an example of using Natural Language Actions API to run the `refreshPluginsCache` action from the `connery-io/connery-runner-administration` plugin.
...
```
curl '<PUBLIC_RUNNER_URL>/v1/actions/run' \
...
Here is an example of how to run the `refreshPluginsCache` action using API.

```
curl '<PUBLIC_RUNNER_URL>/v1/plugins/connery-io/connery-runner-administration@main/actions/refreshPluginsCache/run' \
-H 'Content-Type: application/json' \
-H 'x-api-key: <CONNERY_RUNNER_API_KEY>' \
-d '{}' | jq .
```
...
Now, when you have the runner up and running, let's learn how to [install plugin](/docs/platform/quick-start/install-plugin-on-the-runner) on it.
