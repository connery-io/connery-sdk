...
- [What is a plugin?](/docs/platform/introduction/core-concepts#plugin)
...
For example, for the `connery-io/gmail` plugin, the latest version is `v1.0.0` on the date of writing this article.
We will use this version in the example below.
:::

First, we need to check the plugin definition file [index.js](https://github.com/connery-io/gmail/blob/main/index.js) and find the configuration parameters we need to provide.
...
run the `ClearPluginCache` action of the `connery-io/connery-runner-administration@main` plugin as was shown [on the previous page](/docs/platform/quick-start/set-up-runner#run-action-using-api).
...
Now, when you have the runner configured and the plugins installed, you can learn how to [use clients](/docs/platform/quick-start/use-clients-to-call-actions) to call actions from the plugins.
