...
The connection string consists of key-value pairs separated by a semicolon. The key-value pairs are separated by an equal sign.
...
Multi-connection string consist of multiple connection strings separated by a comma.
The client that uses the multi-connection string has access to all the plugin servers and actions defined in them.

Example of multi-connection string:

```
PLUGIN_SERVER_URL=https://plugin1.com:4201;API_KEY=123456,PLUGIN_SERVER_URL=https://plugin2.com:4201;API_KEY=654321
```
