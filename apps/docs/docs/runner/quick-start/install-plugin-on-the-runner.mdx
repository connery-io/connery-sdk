...
The definition file shows that both `runnerUrl` and `runnerApiKey` configuration parameters are required, so they must be provided in the plugin configuration.
