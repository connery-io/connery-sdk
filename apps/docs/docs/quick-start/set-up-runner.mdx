...
This is the key which the runner uses for the Natural Language Actions feature to access the OpenAI API and classify the user intent. You can get it from [your OpenAI account](https://platform.openai.com/account/api-keys).
This is not required to start the runner, however some clients that are completely based on natural language like [Slack App](/docs/native-clients/slack) won't work without it.
