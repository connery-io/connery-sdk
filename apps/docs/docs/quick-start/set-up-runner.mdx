...
Let's use Natural Language Actions API to run the `RefreshConnectorsCache` from the `connery-io/connery-runner-administration` connector.
...
You should receive a successful response with the result of the action execution and a natural language response.
