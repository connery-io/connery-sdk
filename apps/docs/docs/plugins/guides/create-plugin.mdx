...
to make them available to the community and grow the [ecosystem](/docs/platform/introduction/ecosystem) together :heart:
:::
...
- A new action file will be added to the `./src/actions` folder.
- The new action will be included in the plugin definition in the `./src/index.ts` file.
- A new test file will be added to the `./test/actions` folder.
- The new action will be added to the "Available actions" table in the `./README.md` file.

Now, you can implement the logic of the action. Open the action definition file `./src/actions/<action-name>.ts`,
...
- Check the [plugin definition schema](https://github.com/connery-io/connery-platform/blob/main/packages/lib/src/plugin.schema.ts)
  to understand how the definition of the plugin and action works.
...
You will be asked to select the action you want to run, provide configuration parameters for the plugin, and input parameters for the action.
...
2. [Install the plugin on the runner.](/docs/platform/quick-start/install-plugin-on-the-runner)
3. [Use clients](/docs/platform/quick-start/use-clients-to-call-actions) to run actions from the plugin.
