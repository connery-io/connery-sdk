...
| Run action using natural language | This Make action lets you run action from the runner using natural language, similar to how the [Connery app in Slack](/docs/native-clients/slack#information) works using OpenAI API for classification.<br/><br/>To use the app, you have to configure `OPENAI_API_KEY` in the [runner configuration](http://localhost:4202/docs/getting-started/set-up-runner#3-configure-env-variables) so the Runner can use OpenAI API to classify the user intent and identify input parameters from messages. |
